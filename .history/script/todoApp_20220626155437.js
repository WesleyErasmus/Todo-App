window.addEventListener("load", () => {
  ...
  // const profilePicture = document.querySelector("#file");
  
  // const fileUpload = localStorage.getItem('file') || '';
  
  // profilePicture.value = fileUpload;

  // profilePicture.addEventListener("change", (e) => {
  //   localStorage.getItem('file', e.target.value);
  // })
  ...
  tasksForm.addEventListener("submit", (e) => {
   ...
   if( date.value === "" ) {
    ...
    document.querySelector(".datepicker-toggle").style.color ="red";
    return false;
 } else {
    document.querySelector(".datepicker-toggle").style.visibility="hidden";
    // document.querySelector("#title").style.border ="none";
 }
...
//   fileUpload = image.value;
//   image.src = URL.createObjectURL(event.target.files[0]);
//   localStorage.setItem('file', event.target.value);
// };
...
function dateSort() {
  //So I just used the tasks parameter as I didn't think you needed to create a new param (sortedByDate)
  //Also, the way you were comparing was correct but that wasn't actually using the due date parameter 
    //you chose but more order they were added by the user so would always keep the same order - if that makes sense?
  ...
  //If you are not wanting to save this new order to the localStorage then just remove the below 2 methods.
  localStorage.clear();
  localStorage.setItem("tasks", JSON.stringify(tasks));

  //Lastly, you already have this lovely method which prints the task out nicely to the DOM, so I just called it again after doing the sort.
  printTasksToDOM();
  
  //This would generally work for a an array of simple values but you have objects which is why it was spitting out [object Object], [object Object] etc
