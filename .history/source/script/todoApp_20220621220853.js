...
function printTasksToDOM() {
  ...
  tasks.forEach((task) => {
    ...
    // >>>>> NEW <<<<<<<<<<<<<<
    const categoryPersonal = document.querySelector(".personal-category-btn");
    const categoryWork = document.querySelector(".personal-work-btn");
    // Circle dive that gives a round background below the task submit button
    const circleContainer = document.querySelector(".circle");
    categoryPersonal.addEventListener("click", (e) => {
      //changes the edit icon to a save icon when clicked
      categoryPersonal.style.backgroundColor = "#2093ff2b";
      categoryWork.style.backgroundColor = "#2093ff2b";
      circleContainer.style.backgroundColor = "#2093ff2b";
