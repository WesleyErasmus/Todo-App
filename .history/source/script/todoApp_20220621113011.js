window.addEventListener("load", () => {
  ...
  tasksForm.addEventListener("submit", (e) => {
    ...
    const task = {
      ...
      date: e.target.elements.date.value, //***NEW***
...
function printTasksToDOM() {
  ...
  tasks.forEach((task) => {
    ...
    const dueDate = document.createElement("div"); // *** NEW ***
    ...
    dueDate.classList.add("task-due-date"); // *** NEW ***
    ...
    // Should taskContent be given innerHTML of input with task.title value? FIX!
    ...
    dueDate.innerHTML = "Date due: " + "</br>" + `${task.date}`; // *** NEW ***
    ...
    taskContent.appendChild(dueDate); // *** NEW ***
    ...
    // *** NEW ***
    // Prevents undefined due dates from appearing in tasks

    const newDueDate = new Date().toLocaleDateString('en-us', { weekday:"long", year:"numeric", month:"short", day:"numeric"}).task.date;

    if (dueDate.value === "") {
      dueDate.innerHTML = "";
    } else {
      dueDate.innerHTML = newDueDate;
    }
    ...
    //Don't display "read description button if there is no description"
    ...
    // Sort alphabetically
    // const sortForm = document.getElementById("sort-tasks-form");
