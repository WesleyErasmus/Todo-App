...
function printTasksToDOM() {
    ...
    tasks.forEach(task => {
        ...
        // Sort alphabetically
        const sortForm = document.getElementById("sort-tasks-form").addEventListener("input", nameSort);
        const nameSort = document.getElementById("sort-by-name").addEventListener("input", sortData);
        nameSort.addEventListener((e) => {
            ...
            for (i = 0; i < select.selectedOptions.length; i++) {
                filters.push(select.selectedOptions[i].value);
                console.log(choices);
            }
            })
            function sortData(e) {
